...
Urban/Rural alt dizinleri gerektirmez; düz (flat) dizin yapısından okur.

Beklenen dizin yapısı:
    data_root/
    ├── images_png/          # RGB görüntüler (.png)
    └── masks_png_convert/   # Gri tonlamalı dönüştürülmüş maskeler (.png)
"""
...
class LoveDAFlatTrainDataset(Dataset):
    """Flat directory structure LoveDA dataset for training.

    No Urban/Rural subdirectories required. Reads from:
        {data_root}/images_png/{img_id}.png
    ...
    def __getitem__(self, index):
        ...
        results = {'img': img, 'gt_semantic_seg': mask, 'img_id': img_id, 'img_type': 'all'}
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):
        ...
        if not osp.isdir(img_dir_path):
            raise FileNotFoundError(f"Image directory not found: {img_dir_path}")
        if not osp.isdir(mask_dir_path):
            raise FileNotFoundError(f"Mask directory not found: {mask_dir_path}")

        img_filename_list = os.listdir(img_dir_path)
        mask_filename_list = os.listdir(mask_dir_path)
        assert len(img_filename_list) == len(mask_filename_list), \
            f"Image count ({len(img_filename_list)}) != Mask count ({len(mask_filename_list)})"

        img_ids = [str(f.split('.')[0]) for f in sorted(img_filename_list)]
        return img_ids
...
class LoveDAFlatTestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        results = {'img': img, 'img_id': img_id, 'img_type': 'all'}
        return results
